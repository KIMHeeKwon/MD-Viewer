> ⚠️ **이 문서는 이 저장소의 작업 지침이 아니다.**
> KKOBAK DS 원본 패키지에 딸려 온 *DS 사용 규약*이며, **React 프로젝트를 전제로 쓰였다.**
> 이 앱은 바닐라 렌더러라 **토큰 체계만 채택했고 React 컴포넌트 9종은 채택하지 않았다**
> ([[DECISIONS|D47]]). 따라서 아래의 `ThemeProvider`·컴포넌트 조합 규칙은 여기서 적용되지 않는다.
>
> - 이 프로젝트의 작업 지침: 루트 `CLAUDE.md`
> - DS를 실제로 어떻게 쓰는지: [[DESIGN]] (매핑표·예외·업그레이드 절차)
> - 원래 파일명은 `CLAUDE.md`였다. Claude Code가 프로젝트 지침으로 오독할 수 있어 이름을 바꿨다
>   (2026-08-17).
